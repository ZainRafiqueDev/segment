...
const FAPI = import.meta.env.VITE_FASTAPI || 'http://localhost:8001';
const EXPR = import.meta.env.VITE_EXPRESS || 'http://localhost:5000';
...
───────────────────────────────────────────────────────────────────── */
const glowMap = {
  indigo: 'hover:shadow-indigo-500/20', violet: 'hover:shadow-violet-500/20',
  emerald:'hover:shadow-emerald-500/20', rose:  'hover:shadow-rose-500/20',
  amber:  'hover:shadow-amber-500/20',  teal:  'hover:shadow-teal-500/20', none: '',
};
  ...
  return (
    <motion.div
      ...
      className={`relative bg-white/[0.032] backdrop-blur-md border border-white/[0.07]
                  rounded-2xl shadow-2xl transition-shadow duration-300 ${glowMap[glow]} ${className}`}
    >
      {children}
    </motion.div>
  );
  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between items-center mb-1.5">
        ...
        <motion.span className="text-xs font-bold text-white tabular-nums"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: delay + 0.45 }}>
          {value}%
        </motion.span>
      </div>
      <div className="h-1.5 bg-white/[0.05] rounded-full overflow-hidden">
        <motion.div className={`h-full rounded-full ${gradient}`}
...
function SimilarCard({ item, index }) {
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl p-3 border ${isPos
        ? 'bg-emerald-950/25 border-emerald-800/30'
        : 'bg-rose-950/25 border-rose-800/30'}`}
    >
      <div className="flex justify-between items-center mb-1.5">
      ...
      <div className="mt-2 h-0.5 bg-white/[0.05] rounded-full overflow-hidden">
        <motion.div className={`h-full rounded-full ${isPos ? 'bg-emerald-500' : 'bg-rose-500'}`}
          initial={{ width: 0 }} animate={{ width: `${item.similarity}%` }}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(headerRef.current, { y: -55, opacity: 0 }, { y: 0, opacity: 1, duration: 0.85, ease: 'power3.out' });
    gsap.fromTo(titleRef.current,  { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.6, delay: 0.3, ease: 'back.out(1.5)' });
    gsap.fromTo(badgeRefs.current.filter(Boolean),
      { y: -18, opacity: 0, scale: 0.75 },
      { y: 0, opacity: 1, scale: 1, duration: 0.45, stagger: 0.09, delay: 0.45, ease: 'back.out(2)' }
    );
  }, []);

  const fetchHistory = async () => { try { const { data } = await axios.get(`${EXPR}/api/analyses?limit=8`); setHistory(data); } catch {} };
  const fetchStats   = async () => { try { const { data } = await axios.get(`${EXPR}/api/stats`);            setStats(data);   } catch {} };
  ...
  const analyze = async () => {
    ...
    try {
      ...
      await axios.post(`${EXPR}/api/analyses`, {
        review: review.trim(), sentiment: data.sentiment, confidence: data.confidence,
  ...
  return (
    <div className="min-h-screen bg-[#05050f] text-white relative overflow-x-hidden">
      ...
      <div className="relative" style={{ zIndex: 2 }}>
        ...
        {/* ═══ HEADER ═══ */}
        <header ref={headerRef} className="sticky top-0 border-b border-white/[0.05] bg-[#05050f]/75 backdrop-blur-xl px-6 py-4" style={{ zIndex: 10 }}>
          <div className="max-w-7xl mx-auto flex items-center gap-4">
            ...
            <div ref={titleRef}>
              <h1 className="text-base font-black tracking-tight bg-gradient-to-r from-indigo-300 via-violet-200 to-indigo-300
            ...
            <div className="ml-auto flex flex-wrap gap-2 items-center">
              {badges.map((b, i) => (
                <span key={b.label} ref={el => { badgeRefs.current[i] = el; }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <Card className="p-5" glow="indigo">
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Movie Review</label>
                <textarea value={review} onChange={e => setReview(e.target.value)}
                  placeholder="Paste any movie review here…" rows={7}
                  className="w-full bg-transparent text-slate-200 text-sm resize-none focus:outline-none placeholder-slate-700 leading-relaxed" />
                <div className="flex justify-between mt-2 pt-2 border-t border-white/[0.04] text-xs text-slate-700">
                  <span>{wordCount} words</span><span>{review.length} chars</span>
                </div>
              </Card>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {samples.map((s, i) => (
                    <motion.button key={i} whileHover={{ x: 5 }} whileTap={{ scale: 0.97 }}
              ...
              <motion.button type="button" onClick={analyze} disabled={!review.trim() || loading}
                whileHover={review.trim() && !loading ? { scale: 1.02 } : {}} whileTap={{ scale: 0.97 }}
                className="relative w-full py-4 rounded-xl font-bold text-sm overflow-hidden disabled:opacity-40 disabled:cursor-not-allowed">
                <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-violet-600" />
                <motion.div className="absolute inset-0 bg-gradient-to-r from-indigo-400 to-violet-400"
                      ...
                      <div className="space-y-1.5">
                        {history.map((h, i) => (
                          <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                            ...
                            className="flex items-center justify-between text-xs bg-white/[0.03] border border-white/[0.04] rounded-lg px-3 py-2 gap-2">
              ...
              <AnimatePresence mode="wait">
                {!result ? (
                  <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="flex flex-col items-center justify-center h-64 bg-white/[0.02] border border-white/[0.05] rounded-2xl gap-3">
                    <motion.span animate={{ scale: [1, 1.1, 1], rotate: [0, 5, -5, 0] }}
                      transition={{ duration: 4, repeat: Infinity }} className="text-5xl">🎭</motion.span>
                    <p className="text-sm text-slate-600">Results appear here</p>
                  </motion.div>
                ) : (
                  <motion.div key="result" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }} className="space-y-4">

                    <Card className="p-6 overflow-hidden" glow={isPos ? 'emerald' : 'rose'}>
                      <div className={`absolute inset-0 opacity-[0.08] bg-gradient-to-br pointer-events-none rounded-2xl
                                       ${isPos ? 'from-emerald-400' : 'from-rose-500'} to-transparent`} />
                      ...
                      <div className="flex items-center gap-4">
                        <motion.span initial={{ scale: 0, rotate: -25 }} animate={{ scale: 1, rotate: 0 }}
                          transition={{ type: 'spring', stiffness: 270, delay: 0.1 }} className="text-5xl shrink-0">
                          {isPos ? '😊' : '😞'}
                        </motion.span>
                        <div>
                          <motion.p initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}
                    ...
                    <Card className="p-5" glow="violet">
                      ...
                      <Bar label={`BiLSTM → ${result.lstm_sentiment}`} value={result.lstm_confidence}
                           gradient="bg-gradient-to-r from-indigo-600 to-indigo-400" delay={0} />
                      <Bar label={`BERT → ${result.bert_sentiment}`}   value={result.bert_confidence}
                           gradient="bg-gradient-to-r from-violet-600 to-violet-400" delay={0.18} />
                      <Bar label="Fused (LSTM×0.45 + BERT×0.55)"      value={result.confidence}
                           gradient={isPos ? 'bg-gradient-to-r from-emerald-600 to-emerald-400'
                                           : 'bg-gradient-to-r from-rose-600 to-rose-400'} delay={0.36} />
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}
                        className="text-xs text-slate-600 mt-2 text-center">
                        {result.models_agree ? '✅ Both models agree' : '⚡ Models disagreed — BERT cast the deciding vote'}
                      </motion.p>
                    </Card>
            ...
            <div>
              <AnimatePresence mode="wait">
                {!result ? (
                  <motion.div key="empty-dag" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="bg-white/[0.02] border border-white/[0.05] rounded-2xl p-5 h-48 flex items-center justify-center">
                    <p className="text-xs text-slate-600">DAG pipeline trace appears here</p>
                  </motion.div>
                ) : (
                  <motion.div key="dag" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }}>
                    <Card className="p-5" glow="teal">
                      <p className="text-sm font-bold text-teal-400 mb-0.5">⚙ DAG Pipeline</p>
                      <p className="text-xs text-slate-600 mb-5">8 nodes · no cycles · {result.total_ms}ms total</p>
                      {result.dag_steps.map((step, i) => (
                        <DagNode key={i} step={step} index={i} total={result.dag_steps.length} ready={!!result} />
                      ))}
                    </Card>
                  </motion.div>
                )}
